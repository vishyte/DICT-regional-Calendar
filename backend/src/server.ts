...
async function runMigrations() {
  try {
    ...
    try {
      await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS office_assignment VARCHAR(255)`);
    } catch (err: any) {
    ...
    try {
      ...
      await pool.query(`ALTER TABLE activities ADD COLUMN IF NOT EXISTS requested_status VARCHAR(50)`);
      await pool.query(`ALTER TABLE activities ADD COLUMN IF NOT EXISTS creator_role VARCHAR(255)`);
    } catch (err: any) {
      console.warn('Could not add approval/creator role columns (they may already exist):', err?.message || err);
    }
