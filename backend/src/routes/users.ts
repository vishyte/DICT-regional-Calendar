...
router.post('/register', async (req, res) => {
  try {
    ...
    const existingUser = pool.query('SELECT id FROM users WHERE username = ? OR email = ?', [username, email]);
    ...
    const userResult = pool.query(
      'SELECT id, username, email, first_name, middle_name, last_name, project FROM users WHERE username = ?',
      [username]
    );
...
router.get('/profile', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const result = pool.query('SELECT id, username, email, first_name, middle_name, last_name, project FROM users WHERE id = ?', [req.user!.id]);
    if (result.rows.length === 0) {
